...
export const UserMessageItem: React.FC<UserMessageItemProps> = ({ 
  ...
  if (isEditing) {
      ...
      // User Mode Style
      containerClasses += " bg-blue-600 text-white rounded-tr-sm";
  }
  ...
  return (
    <div 
        className={`flex w-full gap-4 p-4 animate-fade-in group flex-row-reverse`}
    >
      {/* Avatar */}
      <div className={`
      ...
      <div className={`flex flex-col max-w-[85%] lg:max-w-[75%] items-end w-full`}>
              ...
              <div className="flex flex-col gap-1">
                {renderParts.map((part, index) => {
                  switch (part.type) {
                    case 'text':
                      return (
                        <MarkdownRenderer key={index} text={part.text} />
                      );
                    default:
                      return null;
                  }
